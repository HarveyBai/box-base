...
async function loginUser(page: import('@playwright/test').Page, user: string, pass = 'Test1234!') {
...
// ---------------------------------------------------------------------------
test('B1: register → login → logout → re-login → F5 still on /dashboard', async ({ page }) => {
  const user = `b1_${rand()}`
  ...
  const lsRT = await page.evaluate(() => localStorage.getItem('refresh_token'))
...
// B3: 多标签页并发刷新均不掉登录态
// ---------------------------------------------------------------------------
test('B3: multi-tab concurrent reload stays on /dashboard', async ({ page, context }) => {
  const user = `b3_${rand()}`
...
test('B4: dashboard shows tenant name, not UUID', async ({ page }) => {
  ...
  const tenantLabel = page.locator('.ant-descriptions-item-label:has-text("活跃租户")')
  await expect(tenantLabel).toBeVisible({ timeout: 5000 })
  const tenantValue =
    (await tenantLabel.locator('..').locator('.ant-descriptions-item-content').textContent()) ?? ''
...
// ---------------------------------------------------------------------------
test('B5: unauthenticated /dashboard redirects to /login', async ({ page }) => {
...
// ---------------------------------------------------------------------------
test('B6: stale jti refresh returns 401 after grace window', async ({ page }) => {
  const user = `b6_${rand()}`
  ...
  const rt1 = await page.evaluate(() => localStorage.getItem('refresh_token'))
  ...
  const rt2 = await page.evaluate(() => localStorage.getItem('refresh_token'))
  ...
  const resp = await page.request.post('http://localhost:8000/api/auth/refresh', {
