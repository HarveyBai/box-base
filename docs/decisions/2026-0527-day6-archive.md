...
| CI Run #8 frontend-quality job 报 "Unable to locate executable file: pnpm"，根因是 actions/setup-node@v6 的 cache: pnpm 参数需要 pnpm 已在 PATH，但 Run #7 修复时把 setup-node 移到 pnpm/action-setup 之前导致 cache 计算时 pnpm 不存在；这与 pnpm/action-setup v6 self-installer 需要 Node 先就绪形成循环依赖 | CI Run #8 截图反馈 + 日志分析 | 回退 pnpm/action-setup 到 v4（Day 4-5 已验证稳定），恢复原始顺序 checkout → action-setup → setup-node；接受 Node 20 deprecation warning（2026-09-16 前可用）；本条强化纪律：(1) GitHub Actions 版本升级前必须评估循环依赖风险（cache 参数与 action 安装顺序的耦合）；(2) 当多个 action 互为前置条件时，优先选择已验证稳定状态而非追求"零 warning"；(3) Day 6 任务 1 目标设定（消除 deprecation warning）应与"CI 必须绿"目标做权重排序，前者非紧急时不应破坏后者 |
